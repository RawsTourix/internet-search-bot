...
async def seeded_runtime(tmp_path):
    ...
    return repos, FinalizationBarrierService(repositories=repos, clock=lambda: NOW), state
...
async def prepared(tmp_path):
    ...
    assert preparation.record is not None
...
async def result_persisted(tmp_path):
    ...
    record = await service.persist_result(record.finalization_id, {"content": "final", "status": "done"})
...
async def output_ready(tmp_path, output_batch_id: str | None = None):
    ...
    record = await service.mark_output_ready(record.finalization_id, output_batch_id=output_batch_id)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"cycle_status": CycleStatus.PAUSE_REQUESTED, "pending_control_sequence": 1}, "control_before_terminal"),
    ],
)
...
async def test_duplicate_control_without_watermark_change_does_not_phantom_abort(tmp_path):
    ...
    state = await mutate_state(repos, pending_control_sequence=1, applied_control_sequence=1)
    candidate = await service.capture_candidate(session_id="session", cycle_id="cycle")
...
@pytest.mark.asyncio
async def test_waiting_commit_rechecks_input_and_control_and_persists_one_question(tmp_path):
    repos, service, state = await seeded_runtime(tmp_path)
    ...
    assert snapshot.waiting_question == "Need details?"
    ...
    assert (await repos.snapshots.get("cycle")).waiting_question == "Need details?"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"generation": 1, "active_cycle_id": None, "cycle_status": CycleStatus.IDLE, "active_context_revision_id": None}, "waiting_aborted_control"),
    ],
)
...
async def test_post_waiting_input_keeps_same_cycle_authority_for_existing_resume_path(tmp_path):
    ...
    assert updated.active_cycle_id == "cycle"
    assert updated.cycle_status == CycleStatus.WAITING_USER
    assert updated.active_cycle_accepted_through_sequence == 1
...
async def test_result_persisted_record_write_failure_replays_existing_result(tmp_path, monkeypatch):
    ...
    def injected(path, model):
        ...
        if (
            not failed
            and getattr(model, "state", None) == FinalizationState.RESULT_PERSISTED
        ):
            failed = True
...
async def test_output_ready_state_write_failure_replays_same_output_identity(tmp_path, monkeypatch):
    ...
    assert replay.state == FinalizationState.OUTPUT_READY
    assert replay.output_batch_id == output_id
...
async def test_partial_terminal_commit_recreation_converges_without_second_commit(tmp_path, monkeypatch):
    ...
    assert partial.cycle_status == CycleStatus.DONE
    assert partial.finalization_id == record.finalization_id
    assert not await repos.finalizations.output_delivery_allowed(
        session_id="session", cycle_id="cycle", output_batch_id=record.output_batch_id
    )
    monkeypatch.setattr(adapter, "atomic_write_model", original)
...
async def test_real_output_claim_and_outbox_are_fenced_until_terminal_commit(tmp_path):
    ...
    batch = output_batch(new_output_batch_id())
    committed_batch = await store.commit(batch)
    record = await service.mark_output_ready(record.finalization_id, output_batch_id=committed_batch.output_batch_id)
    bind_output_eligibility(service.output_delivery_allowed)
    bind_final_output_assembler(object())
    claim = IdempotentOutputClaimService(store)
    ...
    try:
        ...
        finalization = await service.terminal_commit(record.finalization_id)
        assert finalization.state == FinalizationState.TERMINAL_COMMITTED
        ...
        claimed, attempt = await claim.claim(
            committed_batch.output_batch_id,
            claim_request_id=new_output_claim_request_id(),
            now=NOW,
        )
...
async def test_aborted_output_never_becomes_claimable(tmp_path):
    ...
    repos, service, _, record = await result_persisted(tmp_path)
    store = FileSystemOutputBatchStore(tmp_path)
    batch = await store.commit(output_batch(new_output_batch_id()))
    record = await service.mark_output_ready(record.finalization_id, output_batch_id=batch.output_batch_id)
    await mutate_state(repos, active_cycle_accepted_through_sequence=1)
    aborted = await service.terminal_commit(record.finalization_id)
...
async def test_emission_claim_linearized_before_terminal_commit_remains_legitimate(tmp_path):
    ...
    assert current.delivery_attempt_count == 1
