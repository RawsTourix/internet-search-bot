...
def make_api(tmp_path, *batches):
    ...
    api.input_runtime_repositories = create_filesystem_input_runtime_repositories(storage_config=StorageConfigType(root_dir=str(tmp_path)))
...
async def test_api_uses_admission_cycle_id_and_marks_initial_applied(tmp_path):
    ...
    admission = await api.input_runtime_repositories.admissions.get_by_input_batch_id('initial')
...
async def test_waiting_user_compatibility_resumes_same_cycle_once(tmp_path):
    ...
    assert [call['cycle_id_override'] for call in api.mcp_client.calls] == ['admitted-cycle', 'admitted-cycle']
    admission = await api.input_runtime_repositories.admissions.get_by_input_batch_id('reply')
    assert admission.state.value == 'applied'
    marker = await api.input_admission_service.get_runtime_handoff(admission)
    assert marker is not None
    assert marker.state == RuntimeHandoffState.COMPLETED
    inbox = await api.input_runtime_repositories.inbox.list_for_cycle('admitted-cycle')
    assert len(inbox) == 1
...
class WaitingSideEffectThenFailMCP:
    ...
    async def process_query(self, message, **kwargs):
        self.calls += 1
        if self.calls == 1:
            return AgentResult(content='question', status=AgentStatus.WAITING_USER, session_id=kwargs['session_id'], cycle_id=kwargs['cycle_id_override'])
...
async def test_interrupted_outcome_does_not_automatically_replay_runner(tmp_path):
    ...
    interrupted = await api.admit_committed_batch('addition', session_id='session')
