...
def test_emission_delivery_claim_fenced_and_durable(tmp_path):
 ...
 assert run(repositories(tmp_path).emissions.complete_delivery(e.emission_id,claim_token="claim",delivered_at=NOW)).state==EmissionState.DELIVERED
