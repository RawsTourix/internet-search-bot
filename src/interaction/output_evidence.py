...
class ValidatingOutputDeliveryCompletionService(
    ...
    """Reject contradictory external receipts before touching durable stores."""

    async def complete(self, receipt: OutputDeliveryReceipt):
        batch = await self.output_store.get(receipt.output_batch_id)
        self._validate_external_evidence(batch, receipt)
    ...
    @staticmethod
    def _validate_external_evidence(batch, receipt: OutputDeliveryReceipt) -> None:
